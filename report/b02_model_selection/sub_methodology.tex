...
As 10D networks represented the complexity threshold in Part I at which reconstruction almost failed , the dataset size and trajectory lengths were surprisingly reduced to 10.
In the synchronous mode, a higher sampling frequency ($f=7$) was selected, contrary to previous models.
...
The divergence in \texttt{ATTR\_RATIO} between models (e.g., $0.2$ for 10D sync vs. $1.0$ for 5D sync) is a result of the optimal configurations identified the 1-st part of the project, which demonstrated that:
\begin{itemize}
    ...
    \item \textbf{Asynchronous modes} often require lower attractor ratios to provide the algorithm with enough state-change events to correctly infer the individual node update rules.
    \item \textbf{High-dimensional models} exhibit better reconstruction when sampling frequency is increased, probably by preventing the algorithm from being overwhelmed by the high density of transitions in the state transition system.
\end{itemize}